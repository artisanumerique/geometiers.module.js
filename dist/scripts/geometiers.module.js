...
angular
  ...
  .constant("config", {
       "API": "http://localhost/frameworkgeometiers/api",
  })
  ...
  .filter('formatfilter', ["$filter", function($filter) {
      ...
      return function(str,reverse) {
        ...
        var corres = [
          {name:'nbrentr',value:'Tous les établissements'},
          ...
          {name:'tauxrotation',value:"Immatriculés depuis " + $filter('date')($filter('removeyear')(3),'yyyy')},
...
angular.module('geometiersmodulejsApp')
  .directive('leafletGeometiers', ["Webservice", "tools", "Filtre", "$routeParams", "$rootScope", function (Webservice,tools,Filtre,$routeParams, $rootScope) {
    	  return {
	        ...
	        link: function (scope, element, attrs, ctrl) {
		        ...
		        element.geometiers({
        	        ...
        	        'selectZone': function (params, zone) {ctrl.dessiner(params, zone);
        	        	 $rootScope.$broadcast('map.selectZone',zone);
        	        },
    ...
    var factory = {
      ...
      init : function(params){
        ...
        if(params){
           ...
           delete filtres.idterritoire; 
           delete filtres.decoupage; 
           factory.setFiltres(filtres);

            if(angular.isDefined(params.nomterritoire) 
            && angular.isDefined(params.idterritoire)){
               factory.localisation = {name:params.nomterritoire, value:params.idterritoire};
            }
            
            if(angular.isDefined(params.decoupage)) {
               factory.decoupage = params.decoupage;
            }
  ...
  .directive('searchLocation', ["$resource", "Filtre", "config", function ($resource,Filtre, config) {
    return {
      ...
      link: function postLink(scope, element, attrs) {
		...
		var nbrprop = (attrs.nbr)?attrs.nbr:10;
        	...
        	$(element).autocomplete({
        		...
        		autoFocus: false,
	   		    ...
	   		    source : function(requete, reponse){
	   		    	...
	   		    	var Propositions = $resource(config.API + '/search',
						 params, {
						  query: {method:'GET', cache:true, skipAuthorization: true, isArray:true}
					});

	   		    	var propositions = Propositions.query(function() {
	   		    		scope.loading = false;
		   		    	reponse($.map(propositions.slice(0,nbrprop), function(objet){return objet;}));
	   		    	});				
...
angular.module('geometiersmodulejsApp')
  .directive('filtreGeometiers', ["config", "$resource", "Filtre", "tools", "$timeout", "$filter", function (config, $resource, Filtre, tools, $timeout, $filter) {
    return {
      replace: true,
	  transclude: true,
    ...
    return {
      ...
      link: function postLink(scope, element, attrs) {
	    ...
	    var communeActive = false;
      	...
      	// après selection de zone
	    scope.$on('map.selectZone', function(evt,zone) {
	    	// si zone identique
	    	if(!zone){
